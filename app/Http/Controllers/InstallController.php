...
class InstallController extends Controller
{
    ...
    public function step2(Request $request)
    {
        if (Hash::check('step_2', $request['token'])) {
            // Auto-set purchase code values and proceed to step 3
            Session::put(base64_decode('cHVyY2hhc2Vfa2V5'), 'bypassed');
            Session::put(base64_decode('dXNlcm5hbWU='), 'admin');
    ...
    {
        // Purchase code check removed - auto proceed
        Helpers::setEnvironmentValue('SOFTWARE_ID', 'MzY3NzIxMTI=');
        Helpers::setEnvironmentValue('BUYER_USERNAME', $request['username'] ?? 'admin');
        Helpers::setEnvironmentValue('PURCHASE_CODE', $request['purchase_key'] ?? 'bypassed');

        // Set session values for compatibility
        Session::put(base64_decode('cHVyY2hhc2Vfa2V5'), $request[base64_decode('cHVyY2hhc2Vfa2V5')] ?? 'bypassed');//pk
        Session::put(base64_decode('dXNlcm5hbWU='), $request[base64_decode('dXNlcm5hbWU=')] ?? 'admin');//un
        return redirect('step3?token='.bcrypt('step_3'));
    }
    ...
    public function database_installation(Request $request)
    {
        if (self::check_database_connection($request->DB_HOST, $request->DB_DATABASE, $request->DB_USERNAME, $request->DB_PASSWORD)) {
            ...
            $output = 'APP_NAME=6ammart'.time().
                    ...
                    PURCHASE_CODE=' . (session('purchase_key') ?? 'bypassed') . '
                    BUYER_USERNAME=' . (session('username') ?? 'admin') . '
