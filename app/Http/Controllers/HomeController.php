...
class HomeController extends Controller
{
    ...
    {
        // Activation check removed - auto activate and redirect
        $domain = str_replace(["http://", "https://", "www."], "", url('/'));
        $response = [
            "active" => "1",
            "username" => $request['username'] ?? 'admin',
            "purchase_key" => $request['purchase_key'] ?? 'bypassed',
            "software_id" => env('SOFTWARE_ID', 'MzY3NzIxMTI='),
            "domain" => $domain,
            "software_type" => $request->get('software_type', base64_decode('cHJvZHVjdA==')),
        ];
    ...
    {
        // Activation check removed - always succeed
        $domain = str_replace(["http://", "https://", "www."], "", url('/'));
        $response = [
            "active" => "1",
            "username" => $request['username'] ?? 'admin',
